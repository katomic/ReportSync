...
namespace ReportSync
{
    public partial class ReportSync : Form
    {
        ...
        public ReportSync()
        {
            ...
            bwUpdateConn.DoWork += new DoWorkEventHandler(bwUpdateConn_DoWork);
            bwUpdateConn.ProgressChanged += new ProgressChangedEventHandler(bwUpdateConn_ProgressChanged);
            bwUpdateConn.RunWorkerCompleted += new RunWorkerCompletedEventHandler(bwUpdateConn_RunWorkerCompleted);
        }
        ...
        void bwUpdateConn_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            pbSource.Value = e.ProgressPercentage;
            pbDest.Value = e.ProgressPercentage;
        }

        void bwUpdateConn_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            unCheckTreeNodes(rptSourceTree.Nodes);
            MessageBox.Show("Connections Updated completed successfully.", "Update complete");
        }
        ...
        private void radConn_CheckedChanged(object sender, EventArgs e)
        {
            grpDest.Visible = grpSync.Enabled = btnCopySrcCredentials.Visible = false;

            grpConn.Visible = btnUpdateConn.Enabled = true;
        ...
        {
            grpDest.Visible = grpSync.Enabled = btnCopySrcCredentials.Visible = true;

            grpConn.Visible = btnUpdateConn.Enabled = false;
        }

        string rptDsUsername, rptDsPassword, dsName;

        private void btnUpdateConn_Click(object sender, EventArgs e)
        {


            if (txtRptUserName.Text == "" || txtRptDsPwd.Text == "")
            {
                MessageBox.Show("Please provide the new credentials");
                return;
            }

            if (cmbRptDSName.SelectedItem == null)
            {
                MessageBox.Show("Select a Datasourse from the dropdown to update");
                return;
            }

            rptDsUsername = txtRptUserName.Text;
            rptDsPassword = txtRptDsPwd.Text;
            dsName = cmbRptDSName.SelectedItem.ToString();

            selectedNodeCount = 0;
            checkTreeNodes(rptSourceTree.Nodes, false);
            existingPaths = new List<string>();
            bwUpdateConn.RunWorkerAsync();
        ...
        void bwUpdateConn_DoWork(object sender, DoWorkEventArgs e)
        {
            processedNodeCount = 0;
            var destPath = ROOT_FOLDER;
            if (!String.IsNullOrEmpty(uploadPath))
                destPath = uploadPath;
            UpdateConnections(destPath, rptSourceTree.Nodes);
            bwUpdateConn.ReportProgress(100);
        }
        ...
        private void btnCopySrcCredentials_Click(object sender, EventArgs e)
        {
            txtDestUrl.Text = txtSourceUrl.Text;
            txtDestUser.Text = txtSourceUser.Text;
            txtDestPassword.Text = txtSourcePassword.Text;
        }

        private void convertRdlTo2012ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ConvertTo2012 frmConvert = new ConvertTo2012();
            frmConvert.ShowDialog();
        }

     

        private void btnLoadDSNames_Click(object sender, EventArgs e)
        {
            selectedNodeCount = 0;
            checkTreeNodes(rptSourceTree.Nodes, false);
            existingPaths = new List<string>();
            ...
            if (!String.IsNullOrEmpty(uploadPath))
                destPath = uploadPath;
            LoadDsNames(destPath, rptSourceTree.Nodes);
        }

        private void LoadDsNames(string destPath, TreeNodeCollection nodes)
        {
            foreach (TreeNode node in nodes)
            {
                if ((bool)node.Tag)
                {
                    if (node.Nodes.Count > 0)
                    {
                        ...
                        LoadDsNames(childPath, node.Nodes);
                    }
                    else
                    {
                        ...
                        if (dataSources.Any())
                        {
                            foreach (var rptDS in dataSources)
                            {
                                if (!cmbRptDSName.Items.Contains(rptDS.Name))
                                    cmbRptDSName.Items.Add(rptDS.Name);
                            }
